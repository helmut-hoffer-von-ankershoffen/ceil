...
- [ ] Workshop presentation and education of peers in meetups
...
* Hardware
  ...
  * Rack: transparent
  * Networking: 5-port GBit/s switch + WiFi router for bastion host
  * Power: 6-port USB charger powering switch and Hosts
* Software
...
- The router will manage / route to the subnet `11.0.0.[0-128]` (`11/25`) the K8S nodes will live in and act as their DHCP and DNS server
...
Execute all deployments using `make all-deploy` or deploy step by step as follows:

1) Loadbalancer MetalLB with ARP announcements using helm:  `make metallb-deploy`
2) Ingress Traefik: `make traefik-deploy` and open UI using `make traefik-ui-open`
3) busybox-httpd: `make httpd-deploy` and show webpage using `make httpd-open`
4) Prometheus using helm: `make prometheus-deploy` and open UI using using `make prometheus-open`
5) Grafana using helm: `make grafana-deploy` and open using `make grafana-open`
6) podinfo: `make podinfo-deploy`
7) ngrok: `make ngrok-deploy`
